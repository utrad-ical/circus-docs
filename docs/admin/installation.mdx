...
1. コンテナ内部でコマンドを実行するため、CIRCUS の Docker コンテナを作成し、内部で `/bin/bash` を実行します。
   ...
   ```shell-session
   $ sudo docker run --rm -v /var/circus/data:/var/circus/data -v /var/run/docker.sock:/var/run/docker.sock -it circuscad/circus:latest
   ```
...
```shell-session
$ sudo docker run --rm -v /var/circus/data:/var/circus/data -v /var/run/docker.sock:/var/run/docker.sock -p 80:80 -p 27017:27017 --add-host=hostmachine:[IP address for host] -it circuscad/circus:latest
```
...
```shell-session
$ sudo docker run --rm -v /var/circus/data:/var/circus/data -v /var/run/docker.sock:/var/run/docker.sock -p 80:80 -p 27017:27017 --add-host=hostmachine:[IP address for host] -d -e DAEMON_MODE=1 circuscad/circus:latest
```
...
1. ログイン画⾯が表⽰されたら、ユーザ名／パスワードともに "circus" でログインしてください。

1. CIRCUS のホーム画⾯が表⽰されれば成功です。

:::warning

ユーザ "circus" は初期設定ユーザです。ログイン後、[ユーザの設定](user-settings.md) でパスワードを変更してください。必要に応じて、別の管理者権限を持つユーザを作成し、そのユーザでログインし直して "circus" のログインを無効にしてください。
