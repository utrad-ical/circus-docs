...
インストールの際に BIOS の設定変更を求められることがあります。
...
```shell-session
$ sudo docker pull circuscad/circus:1.0.0-beta10
...
1. コンテナ内部でコマンドを実行するため、CIRCUS の Docker コンテナを作成し、内部で `/bin/bash` を実行します。
   ...
   ```shell-session
   $ sudo docker run --rm -v /var/circus/data:/var/circus/data -v /var/run/docker.sock:/var/run/docker.sock -it circuscad/circus:1.0.0-beta10
   ```
...
```shell-session
$ sudo docker run --rm -v /var/circus/data:/var/circus/data -v /var/run/docker.sock:/var/run/docker.sock -p 80:80 -p 27017:27017 --add-host=hostmachine:[IP address for host] -it circuscad/circus:1.0.0-beta10
```
...
起動後にウェブブラウザにて http://localhost/ にアクセスします。
...
```shell-session
$ sudo docker run --rm -v /var/circus/data:/var/circus/data -v /var/run/docker.sock:/var/run/docker.sock -p 80:80 -p 27017:27017 --add-host=hostmachine:[IP address for host] -d -e DAEMON_MODE=1 circuscad/circus:1.0.0-beta10
```
