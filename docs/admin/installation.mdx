...
1. CIRCUS の Docker コンテナに 内で以下のコマンドを実行し、データベースの初期設定を行います。
...
1.  Docker イメージの一部設定を変更し、新たなイメージとして保存します。
    ...
    1.  (Windows) Docker コンテナの ID を新たな Docker image(circuscad/circus-for-win:1.0.0-beta8)として保存します。
...
1.  上記操作で新たに作成した Docker image 内で以下のコマンドを実行し、データベースなどを構築します。
