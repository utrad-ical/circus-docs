...
"circus" is the initial user with all administrative privileges. After logging-in, make sure to change its password following the steps described in [User Settings](user-settings.md). Additionally, you can disable the "circus" account after creating another user with full administrative privileges.
