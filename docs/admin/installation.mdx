...
:::tip
- Linuxではdockerの使用は基本的にはsudo権限が必要ですが、以下の説明では`sudo`の記載を省略しています。
...
1. コンテナ内部でコマンドを実行するため、CIRCUS のDocker imageを使って /bin/bash を実行します。
        ...
        # docker run --rm -v /var/circus/data:/var/circus/data -v /var/run/docker.sock:/var/run/docker.sock -it circuscad/circus:1.0.0-beta8 /bin/bash
...
以下の手順はホスト側のCIRCUSのデータフォルダをC:\circus\data（以下ではWSL2での記法に倣って、/C/circus/dataと記述）設定します。
...
- コンテナ内部でコマンドを実行する場合(/bin/bash を起動)
      ...
      # docker run --rm -v /var/circus/data:/var/circus/data -v /var/run/docker.sock:/var/run/docker.sock -p 80:80 -p 27017:27017 --add-host=hostmachine:[IP address for host] -it circuscad/circus:1.0.0-beta8 /bin/bash
...
- デーモンとして起動させる場合(サーバ群が自動起動します)
      ...
      # docker run -d --rm -v /var/circus/data:/var/circus/data -v /var/run/docker.sock:/var/run/docker.sock -p 80:80 -p 27017:27017 --add-host=hostmachine:[IP address for host] circuscad/circus:1.0.0-beta8
